...
def _prepare_case(root: Path) -> None:
    _write(
        ...
        """---
title: Quality Repair Active Questions Smoke
...
- source_trigger: learning quality repair smoke
- target: memory-emotion
- urgency: medium
- emotional_weight: 70
- status: partially_answered
- next_action: source diversity repair
""",
    )
    _write(
        root / "memory/knowledge/learning_quality_state.md",
        """---
title: Learning Quality Repair Smoke
...
- mode: source_quality_repair_smoke_quality
...
""",
    ...
    _write(
        ...
        """---
title: Source Gate Repair Smoke
...
""",
    ...
    _write(
        ...
        """---
title: Source Reliability Repair Smoke
...
""",
    ...
    _write(
        ...
        """---
title: Source Requests Repair Smoke
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Validate learning-quality-driven source repair planning.")
...
def main() -> int:
    ...
    integration_result = {"integration_permission": "unknown", "quality_repair_candidates": 0}
    planner_result = {"planned_requests": 0, "pending_url_requests": 0}
    protected_changed: list[str] = []

    try:
        _prepare_case(root)
        integration_result = run_source_integration_gate(root, mode="source_quality_repair_smoke_integration")
        planner_result = run_source_request_planner(root, mode="source_quality_repair_smoke_planner")
        request_text = (root / "memory/knowledge/source_requests.md").read_text(encoding="utf-8-sig")
        repair_written = "repair_kind: source_diversity" in request_text and "avoid_host: alpha.example" in request_text
    ...
    if protected_changed:
        return 5
    if args.require_repair and (
        integration_result["integration_permission"] != "prepare_only"
        or int(integration_result["quality_repair_candidates"]) <= 0
