...
CHECKED_FILES = [
    ...
    "test-inputs/late_night_closeness.txt",
    "test-inputs/memory_closure.txt",
    "test-inputs/night_return.txt",
    "TEST-SCENARIOS.md",
...
REQUIRED_OUTPUT_SNIPPETS = [
    ...
    "我会接住你",
    "你这样靠近，我会反应慢一点。",
    "You may keep part of a feeling unsaid.",
]


REQUIRED_SYSTEM_SNIPPETS = [
    "A live user turn is not complete until visible outward text has been sent",
    "use at most 2 synchronous writer calls before output",
    "answer now in direct plain text",
...
def main() -> int:
    ...
    if not good_examples.strip():
        failures.append("prompts/output.md: good tone examples section missing")
    ...
    if "never place forbidden comfort templates in positive examples" not in system_text:
        failures.append("prompts/system.md: positive-example guard missing")
