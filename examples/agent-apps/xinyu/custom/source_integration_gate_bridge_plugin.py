...
def _trace(root: Path, line: str) -> None:
    trace_path = root / "memory/knowledge/source_integration_gate_trace.log"
...
class SourceIntegrationGateBridgePlugin(BasePlugin):
    ...
    async def post_llm_call(
        ...
        try:
            ...
            result = run_source_integration_gate(
                root,
                checked_at=checked_at,
                mode="runtime_source_integration_gate",
            )
            self._ctx.set_state("source_integration_gate_last_run", checked_at)
            _trace(
                root,
                "runtime_source_integration_gate "
