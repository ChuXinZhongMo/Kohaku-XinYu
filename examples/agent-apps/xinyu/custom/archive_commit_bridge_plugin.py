"""Low-frequency runtime bridge for Xinyu archive commit state."""
...
def _trace(root: Path, line: str) -> None:
    trace_path = root / "memory/archive/archive_commit_trace.log"
...
class ArchiveCommitBridgePlugin(BasePlugin):
    name = "xinyu_archive_commit_bridge"
    priority = 106
    ...
    def __init__(self, options: dict[str, Any] | None = None, **_: Any):
        ...
        self._min_interval_seconds = int(opts.get("min_interval_seconds", 9900))
    ...
    def _should_run(self, root: Path) -> tuple[bool, str]:
        ...
        if "- archive_commit: yes" not in recommendations:
            return False, "recommendation_not_yes"

        last_run = self._ctx.get_state("archive_commit_last_run")
        if last_run:
            try:
    ...
    async def post_llm_call(
        ...
        try:
            ...
            result = run_archive_commit(
                root,
                checked_at=checked_at,
                mode="runtime_archive_commit",
            )
            self._ctx.set_state("archive_commit_last_run", checked_at)
            _trace(
                root,
                "runtime_archive_commit "
                f"action={result['commit_action']} "
                f"reason={result['commit_reason']} "
                f"committed={result['committed_items']}",
            )
