...
def _trace(root: Path, line: str) -> None:
    trace_path = root / "memory/self/personality_growth_gate_trace.log"
...
class PersonalityGrowthGateBridgePlugin(BasePlugin):
    ...
    def _should_run(self, root: Path) -> tuple[bool, str]:
        ...
        if "- personality_growth_gate: yes" not in recommendations:
            return False, "recommendation_not_yes"

        last_run = self._ctx.get_state("personality_growth_gate_last_run")
        if last_run:
            try:
