...
def _trace(root: Path, line: str) -> None:
    trace_path = root / "memory/knowledge/source_reliability_trace.log"
...
class SourceReliabilityBridgePlugin(BasePlugin):
    ...
    def _should_run(self, root: Path) -> tuple[bool, str]:
        ...
        if "- deferred: source_gate" not in dispatch:
            return False, "dispatch_not_source_gate"
        ...
        last_run = self._ctx.get_state("source_reliability_last_run")
        if last_run:
            try:
    ...
    async def post_llm_call(
        ...
        try:
            ...
            self._ctx.set_state("source_reliability_last_run", checked_at)
            _trace(
                root,
                "runtime_source_reliability "
