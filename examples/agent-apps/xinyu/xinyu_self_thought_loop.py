    ...
    *,
    ...
    for item in sorted(items, key=_reflection_item_score, reverse=True):
        ...
        if not strong_enough:
            continue
        if _reflection_codex_topic(joined := f"{topic} {item.get('waking_residue', '')}") and not _current_codex_status_supports_reflection(snapshot):
    ...
    if len(parts) >= 3:
        for index in range(1, len(parts), 2):
            ...
            items.append(
                {
                    "item_id": item_id,
                    "topic": _extract_value(body, "topic", "none"),
                    "source": _extract_value(body, "source", "none"),
                    "priority": _extract_value(body, "priority", "none"),
                    "waking_residue": _extract_value(body, "waking_residue", "none"),
...
def _reflection_architecture_defect_message(joined: str) -> str | None:
    ...
    ) or any(marker in joined for marker in ("模板", "客服", "机械", "AI味", "不像人"))
...
def _reflection_share_message(item: dict[str, str]) -> str:
    ...
    if any(marker in joined for marker in ("模板", "客服", "机械", "AI味", "不像人")):
    ...
    if any(marker in joined for marker in ("靠近", "想靠近", "回到身边")):
        return "关于靠近和别像客服那块残留还在。我想确认：你要我更主动贴近一点，还是少说点但别断开？"
    if topic in {"", "none", "unknown"}:
        return "none"
...
def _shareable_dream(snapshot: dict[str, Any], dream_id: str) -> dict[str, str] | None:
    ...
    surface = _extract_value(dream_body, "dream_surface", "")
    fragments = _extract_value(dream_body, "fragments", "")
    if surface in {"", "none", "unknown"}:
        surface = _extract_value(thought_seeds, "latest_fragments", "")
    if fragments in {"", "none", "unknown"}:
        fragments = _extract_value(thought_seeds, "latest_fragments", "")
    boundary = _extract_value(dream_body, "reality_boundary_check", "")
    if boundary in {"", "none", "unknown"}:
        boundary = _extract_value(thought_seeds, "reality_boundary", "")
...
def _dream_share_message(share: dict[str, str]) -> str:
    ...
    return _one_line(
        f"我刚才有个梦想跟你说：{surface}。我知道这只是梦，不是现实新发生的事。",
