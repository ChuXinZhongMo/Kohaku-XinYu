...
| QQ config/sender/command extraction | `python -m py_compile xinyu_qq_gateway.py <new-module>`; `xinyu_qq_config_smoke.py` for route derivation; `qq_config_helpers_smoke.py` for config parsing helpers; `xinyu_qq_gateway_smoke.py`; `xinyu_qq_review_smoke.py` |
