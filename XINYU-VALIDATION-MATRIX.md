...
| Bridge value helper boundary | `python -m py_compile xinyu_bridge_values.py xinyu_core_bridge.py bridge_values_smoke.py`; `bridge_values_smoke.py`; `bridge_session_smoke.py` when session key aliases are touched; `bridge_probe_smoke.py` |
| Bridge state text helper boundary | `python -m py_compile xinyu_bridge_state_text.py xinyu_core_bridge.py bridge_state_text_smoke.py`; `bridge_state_text_smoke.py`; `autonomous_state_smoke.py` when timestamp helpers are touched; `bridge_probe_smoke.py` |
...
| Bridge desktop service aliases | `python -m py_compile bridge_desktop_service_aliases_smoke.py xinyu_core_bridge.py xinyu_desktop_service.py`; `bridge_desktop_service_aliases_smoke.py`; `service_boundary_smoke.py`; `xinyu_desktop_rest_smoke.py`; `bridge_probe_smoke.py` |
...
| Bridge payload privacy policy | `python -m py_compile xinyu_bridge_payload_policy.py bridge_payload_policy_smoke.py xinyu_core_bridge.py`; `bridge_payload_policy_smoke.py`; focused owner/trusted payload pytest cases; `dialogue_privacy_scope_smoke.py`; `xinyu_v1_owner_simple_canary_smoke.py`; `bridge_probe_smoke.py` |
...
| Bridge renderer helpers/guard flags/mode aliases | `python -m py_compile xinyu_bridge_renderer.py bridge_renderer_guard_flags_smoke.py xinyu_core_bridge.py`; `bridge_renderer_guard_flags_smoke.py`; `xinyu_speech_controller_smoke.py`; focused final-guard pytest cases; `bridge_probe_smoke.py` |
...
| Bridge trusted public search policy | `python -m py_compile xinyu_bridge_trusted_search.py bridge_trusted_search_smoke.py xinyu_core_bridge.py`; `bridge_trusted_search_smoke.py`; focused trusted-public-search pytest; `runtime_security_smoke.py`; `codex_delegate_smoke.py`; `bridge_probe_smoke.py` |
...
| Recent attachment context state | `python -m py_compile xinyu_recent_attachment_context.py recent_attachment_context_smoke.py state_service.py state_io_smoke.py`; `recent_attachment_context_smoke.py`; `state_io_smoke.py`; `python -m pytest tests\test_dialogue_curiosity_bridge_injection.py -q` |
| Runtime failure freshness helper | `python -m py_compile xinyu_runtime_failure_freshness.py runtime_failure_freshness_smoke.py`; `runtime_failure_freshness_smoke.py`; `proactivity_scorer_smoke.py`; `impulse_soup_smoke.py`; `python -m pytest tests\test_runtime_program_awareness.py -q` |
| QQ outbox state summary helper | `python -m py_compile xinyu_qq_outbox_state.py xinyu_qq_outbox.py xinyu_runtime_presence.py`; `qq_outbox_smoke.py`; `python -m pytest tests\test_runtime_program_awareness.py -q` |
...
| QQ reply bubble helpers | `python -m py_compile xinyu_qq_gateway.py xinyu_qq_reply_bubbles.py xinyu_qq_gateway_smoke.py`; `xinyu_qq_gateway_smoke.py`; `xinyu_qq_review_smoke.py` |
| QQ normalizer aliases | `python -m py_compile xinyu_qq_gateway.py qq_normalizer_aliases_smoke.py xinyu_qq_normalizer.py`; `qq_normalizer_aliases_smoke.py`; `xinyu_qq_gateway_smoke.py`; `xinyu_qq_review_smoke.py` |
| QQ outbox client aliases | `python -m py_compile xinyu_qq_gateway.py qq_outbox_route_alias_smoke.py xinyu_qq_outbox_client.py`; `qq_outbox_route_alias_smoke.py`; `python -m pytest tests\test_gateway_ack_spool.py -q`; `xinyu_qq_gateway_smoke.py`; `xinyu_qq_review_smoke.py` |
| QQ trust command aliases | `python -m py_compile xinyu_qq_gateway.py qq_trust_aliases_smoke.py xinyu_qq_trust_policy.py`; `qq_trust_aliases_smoke.py`; `qq_trust_policy_smoke.py`; `xinyu_qq_gateway_smoke.py`; `xinyu_qq_review_smoke.py` |
