...
Compatibility loaders now resolve workspace-level `assets/cases/conversation/`
first, then fall back to the older `cases/conversation/` path and the legacy
live path. Do not hard-delete the legacy path until imports, tests, and fixture
loaders no longer reference it.
