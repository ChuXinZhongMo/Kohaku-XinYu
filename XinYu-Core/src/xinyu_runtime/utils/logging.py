...
Default behavior:
  - Logs written to ``~/.xinyu/logs`` (rotating, 10MB x 5)
  - No stderr output by default, keeps CLI clean
  - Set ``XINYU_LOG_STDERR=1`` to also log to stderr (legacy
    ``KT_LOG_STDERR=1`` still works)
...
PRIMARY_LOGGER_NAME = "xinyu_runtime"
RUNTIME_LOGGER_NAMES = (PRIMARY_LOGGER_NAME,)


DEFAULT_LOG_DIR = xinyu_path("logs")


def _runtime_roots() -> list[logging.Logger]:
    """Return runtime logger roots."""
    return [logging.getLogger(name) for name in RUNTIME_LOGGER_NAMES]
...
def _remove_handler_if_present(logger: logging.Logger, handler: logging.Handler) -> None:
...
def configure_utf8_stdio(*, log: bool = False) -> None:
    ...
    if log:
        logger = logging.getLogger(f"{PRIMARY_LOGGER_NAME}.startup")
        logger.info(
...
def _make_log_filename() -> str:
    ...
    when multiple XinYu Runtime instances run concurrently.
    """
...
def get_logger(name: str, level: int | str = logging.INFO) -> logging.Logger:
    ...
    By default, logs go to ``~/.xinyu/logs`` (rotating).
    Set ``XINYU_LOG_STDERR=1`` to also log to stderr.
    ...
    if _handler is None:
        ...
        for root_logger in _runtime_roots():
            _add_handler_once(root_logger, _handler)

        # Optional: stderr handler if XINYU_LOG_STDERR=1 (or legacy KT_LOG_STDERR=1)
        if os.environ.get("XINYU_LOG_STDERR") or os.environ.get("KT_LOG_STDERR"):
            stderr_handler = FlushingStreamHandler(sys.stderr)
            stderr_handler.setFormatter(ColoredFormatter(use_color=True))
            stderr_handler.setLevel(logging.DEBUG)
            for root_logger in _runtime_roots():
                _add_handler_once(root_logger, stderr_handler)
...
def set_level(level: int | str) -> None:
    """
    Set global logging level for all XinYu runtime loggers.
...
def enable_stderr_logging(level: int | str = logging.DEBUG) -> None:
    ...
    _stderr_handler.setLevel(level)
    for root_logger in _runtime_roots():
        _add_handler_once(root_logger, _stderr_handler)
...
def disable_stderr_logging() -> None:
    ...
    if _stderr_handler is None:
        return
    for root_logger in _runtime_roots():
        _remove_handler_if_present(root_logger, _stderr_handler)
...
def enable_tui_logging(write_func: Any) -> None:
    ...
    for root_logger in _runtime_roots():
        _add_handler_once(root_logger, _tui_handler)
...
def disable_tui_logging() -> None:
    ...
    if _tui_handler:
        for root_logger in _runtime_roots():
            _remove_handler_if_present(root_logger, _tui_handler)
        _tui_handler = None
