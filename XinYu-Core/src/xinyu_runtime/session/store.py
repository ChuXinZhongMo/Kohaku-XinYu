"""
SessionStore - persistent session storage backed by the XinYu session store.

Single .xinyu file (SQLite) containing:
...
from xinyu_runtime.session.artifacts import artifacts_dir_for, write_artifact_bytes
from xinyu_runtime.session.history import normalize_resumable_events
...
class SessionStore:
    """Persistent session storage backed by the XinYu session store.
    ...
    def artifacts_dir(self) -> Path:
        ...
        Lives alongside the ``.xinyu`` file at
