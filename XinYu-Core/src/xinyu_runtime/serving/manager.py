...
All runtime operations go through XinYuManager.
...
from xinyu_runtime.core.channel import AgentChannel, ChannelMessage
from xinyu_runtime.core.config import AgentConfig
from xinyu_runtime.core.environment import Environment
from xinyu_runtime.modules.user_command.base import UserCommandContext
from xinyu_runtime.packages import is_package_ref, resolve_package_path
from xinyu_runtime.serving.agent_session import AgentSession
from xinyu_runtime.serving.events import ChannelEvent
...
class XinYuManager:
    ...
    async def agent_create(
        ...
        if self._session_dir:
            try:
                session_path = Path(self._session_dir) / f"{session.agent_id}.xinyu"
    ...
    async def terrarium_create(
        ...
        if self._session_dir:
            try:
                session_path = Path(self._session_dir) / f"{terrarium_id}.xinyu"
                store = SessionStore(session_path)
                store.init_meta(
