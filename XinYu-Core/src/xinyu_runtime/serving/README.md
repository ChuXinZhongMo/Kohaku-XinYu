# serving/

Core service API for hosting and managing agents and terrariums.
Transport-agnostic: used by any interface layer (CLI, TUI, Web API, Gradio).
`XinYuManager` is the single entry point for all runtime operations,
including agent lifecycle, terrarium lifecycle, creature hot-plug, and
channel interactions. `AgentSession` wraps a standalone agent with streaming
chat. Event types are plain dataclasses usable across transport boundaries.

## Files

| File | Description |
|------|-------------|
| `__init__.py` | Re-exports `XinYuManager`, `AgentSession`, `OutputEvent`, `ChannelEvent` |
| `manager.py` | `XinYuManager`: unified service manager with `agent_*`, `terrarium_*`, `creature_*` method hierarchy |
| `agent_session.py` | `AgentSession`: wraps Agent with input injection and async output streaming |
| `events.py` | `OutputEvent` (text chunk, tool activity) and `ChannelEvent` (observed channel message) dataclasses |

## Dependencies

- `xinyu_runtime.core.agent` (Agent)
- `xinyu_runtime.core.channel` (AgentChannel, ChannelMessage)
- `xinyu_runtime.core.config` (AgentConfig)
- `xinyu_runtime.core.environment` (Environment)
- `xinyu_runtime.session.store` (SessionStore)
- `xinyu_runtime.terrarium` (TerrariumRuntime, TerrariumConfig, ChannelObserver)
- `xinyu_runtime.utils.logging`

