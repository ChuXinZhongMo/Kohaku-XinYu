"""
Web server and desktop app launcher for XinYu Runtime.
...
from xinyu_runtime.utils.logging import configure_utf8_stdio, get_logger, set_level
from xinyu_runtime.utils.paths import xinyu_path

logger = get_logger(__name__)

# web_dist lives at src/xinyu_runtime/web_dist/ (built by vite)
...
def _resolve_config_dirs() -> tuple[list[str], list[str]]:
    ...
    Sources (all merged):
      1. XINYU_CREATURES_DIRS / XINYU_TERRARIUMS_DIRS env vars
         (legacy KT_CREATURES_DIRS / KT_TERRARIUMS_DIRS still work)
      2. Installed packages (``~/.xinyu/packages/`` with legacy fallback)
      3. Local project dirs (``creatures/``, ``terrariums/`` in project root)
    """
    from xinyu_runtime.packages import _get_package_root, list_packages

    creatures: list[str] = []
    terrariums: list[str] = []

    # 1. Env vars (highest priority, explicit override)
    env_creatures = os.environ.get("XINYU_CREATURES_DIRS") or os.environ.get(
        "KT_CREATURES_DIRS"
    )
    if env_creatures:
        creatures.extend(env_creatures.split(","))
    env_terrariums = os.environ.get("XINYU_TERRARIUMS_DIRS") or os.environ.get(
        "KT_TERRARIUMS_DIRS"
    )
    if env_terrariums:
...
def run_web_server(
    ...
    if not dev and not (static_dir and static_dir.is_dir()):
        logger.error(
            "web_dist not found - build the XinYu web frontend first, "
            "or use --dev mode",
    ...
    if dev:
        ...
        print(
            ...
            "npm run dev --prefix <xinyu-web-frontend>"
        )
    else:
        print(f"XinYu Runtime web UI: http://{host}:{port}")
...
def run_desktop_app(port: int = 8001, log_level: str = "INFO") -> None:
    ...
    Stderr is redirected to ``~/.xinyu/app.log`` for debugging.
    """
    import subprocess
    ...
    print(f"XinYu Runtime desktop app launched (port {port})")
...
def _run_desktop_app_blocking(port: int = 8001, log_level: str = "INFO") -> None:
    ...
    try:
        ...
        print("pywebview is required for 'xinyu-runtime app'.")
        print("Install: pip install XinYuTerrariumRuntime")
        sys.exit(1)
    ...
    def _set_icon_windows():
        try:
            ...
            if not hicon:
                return
            hwnd = user32.FindWindowW(None, "XinYu Runtime")
            if hwnd:
