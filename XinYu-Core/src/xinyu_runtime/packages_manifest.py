...
def _scan_manifest_field(
    ...
    if len(matches) > 1:
        ...
        raise ValueError(
            ...
            f"in its xinyu.yaml to resolve the conflict."
        )
    return matches[0]
...
def resolve_package_prompt(name: str) -> Path | None:
    ...
    Callers (typically :mod:`xinyu_runtime.prompt.template`) feed the
...
def _package_root_for(pkg_name: str) -> Path | None:
    ...
    Delegates to :func:`xinyu_runtime.packages.get_package_path` but
