...
from xinyu_runtime.builtins.tui.output import TUIOutput
from xinyu_runtime.builtins.tui.session import TUISession
from xinyu_runtime.builtins.tui.widgets import ChatInput
...
from xinyu_runtime.terrarium.cli_output import (
    CLIOutput,
    _format_ts,
    _print_channel_message,
)
from xinyu_runtime.terrarium.config import load_terrarium_config
from xinyu_runtime.terrarium.observer import ChannelObserver
...
def add_terrarium_subparser(subparsers: argparse._SubParsersAction) -> None:
    ...
    run_p.add_argument(
        ...
        const="__auto__",
        default="__auto__",
        help="Session file path (default: auto in ~/.xinyu/sessions/)",
    )
    run_p.add_argument(
        "--no-session",
        action="store_true",
...
def handle_terrarium_command(args: argparse.Namespace) -> int:
    ...
    match args.terrarium_command:
        ...
        case _:
            print("Usage: xinyu-runtime terrarium {run,info}")
            return 0
...
def _run_terrarium_cli(args: argparse.Namespace) -> int:
    ...
    if session_arg is not None:
        if session_arg == "__auto__":
            ...
            session_file = _session_dir / f"{config.name}_{id(config):08x}.xinyu"
        else:
