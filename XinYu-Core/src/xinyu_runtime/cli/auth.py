...
from xinyu_runtime.llm.codex_auth import CodexTokens, oauth_login
from xinyu_runtime.llm.profiles import get_api_key, load_backends, save_api_key
...
def _login_codex() -> int:
    ...
    if existing and not existing.is_expired():
        ...
        print(
            f"Token path: {existing._path if hasattr(existing, '_path') else '~/.xinyu/codex-auth.json'}"
        )
    ...
    try:
        ...
        print("Tokens saved to: ~/.xinyu/codex-auth.json")
        return 0
