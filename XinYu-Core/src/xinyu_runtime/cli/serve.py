...
from xinyu_runtime.cli.version import get_git_info, get_package_version
from xinyu_runtime.serving.web import run_web_server
from xinyu_runtime.utils.paths import xinyu_path

RUN_DIR = xinyu_path("run")
...
def serve_start_cli(args: argparse.Namespace) -> int:
    ...
    if runtime["alive"]:
        state = runtime["state"]
        print("XinYu Runtime web daemon is already running")
    ...
    if not _wait_until_alive(pid):
        print("Failed to start XinYu Runtime web daemon.")
    ...
    state = _load_state()
    print("XinYu Runtime web daemon started")
    print(f"  pid:  {pid}")
    print(f"  url:  {state.get('url', '')}")
...
def serve_stop_cli(args: argparse.Namespace) -> int:
    ...
    if not pid or not runtime["alive"]:
        _remove_runtime_files()
        print("XinYu Runtime web daemon is not running.")
        return 0
...
def serve_status_cli() -> int:
    ...
    pid = runtime["pid"]
    if not pid:
        print("XinYu Runtime web daemon: stopped")
        print(f"  log: {LOG_PATH}")
        return 0

    if not runtime["alive"]:
        print("XinYu Runtime web daemon: stale state")
    ...
    print("XinYu Runtime web daemon: running")
    print(f"  pid:          {pid}")
    if state.get("url"):
        print(f"  url:          {state['url']}")
