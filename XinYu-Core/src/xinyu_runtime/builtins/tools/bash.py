...
def _shell_override_env(shell_type: str) -> str | None:
    specific = os.environ.get(f"XINYU_{shell_type.upper()}_PATH") or os.environ.get(
        f"KT_{shell_type.upper()}_PATH"
    )
    if specific:
        return specific
    generic = os.environ.get("XINYU_SHELL_PATH") or os.environ.get("KT_SHELL_PATH")
    if generic:
        return generic
...
def _create_output_file() -> tuple[Path, Any]:
    output_dir = Path(tempfile.gettempdir()) / "xinyu-runtime-bash"
