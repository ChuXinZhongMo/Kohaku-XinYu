...
from xinyu_runtime.commands.base import BaseCommand, CommandResult, parse_command_args

if TYPE_CHECKING:
    from xinyu_runtime.prompt.skill_loader import SkillDoc
...
def _render_skill_from_path(path: Path) -> str | None:
    ...
    """
    # Lazy import: ``xinyu_runtime.prompt.__init__`` eagerly loads the
    # aggregator, which imports from ``core`` -> ``commands.read``.
    # Importing at call time breaks that cycle.
    from xinyu_runtime.prompt.skill_loader import load_skill_doc
