"""
Package manager for XinYu Runtime creature/terrarium packages.
...
Editable installs write a pointer file instead of copying:
  ~/.xinyu/packages/<name>.link   (contains absolute path)
...
PACKAGES_DIR = xinyu_path("packages")
LINK_SUFFIX = ".link"


def _package_dirs_for_read() -> list[Path]:
    dirs: list[Path] = []
    for directory in [PACKAGES_DIR, *readable_dirs("packages")]:
        if directory not in dirs and directory.exists():
            dirs.append(directory)
    return dirs or [PACKAGES_DIR]
...
def _read_link(name: str) -> Path | None:
    ...
    link_file = next(
        (
            package_dir / f"{name}{LINK_SUFFIX}"
            for package_dir in _package_dirs_for_read()
            if (package_dir / f"{name}{LINK_SUFFIX}").exists()
        ),
        None,
    )
...
def install_package(
    ...
    Args:
        ...
        name_override: Override package name (default: from xinyu.yaml or dir name).
...
def list_packages() -> list[dict]:
    ...
    package_dirs = _package_dirs_for_read()
    if not package_dirs:
        return []
    ...
    for package_dir in package_dirs:
        ...
        for entry in entries:
            package = _package_entry(entry, seen)
...
def _package_entry(entry: Path, seen: set[str]) -> dict | None:
    ...
    return {
        ...
        # scan both without two round-trips through list_packages().
...
def _resolve_manifest_entry(
    ...
    if len(matches) > 1:
        ...
        raise ValueError(
            ...
            f"xinyu.yaml to resolve the conflict."
        )
...
def resolve_package_io(io_name: str) -> tuple[str, str] | None:
    ...
    Looks up ``io:`` entries declared in each package's ``xinyu.yaml``.
...
def resolve_package_trigger(trigger_name: str) -> tuple[str, str] | None:
    ...
    Looks up ``triggers:`` entries declared in each package's ``xinyu.yaml``.
    Collisions across packages raise ``ValueError`` at lookup time.
...
# single import surface (``from xinyu_runtime.packages import
# resolve_package_skills``). Safe against circular import because the
# re-export runs after ``list_packages`` is defined.
from xinyu_runtime.packages_manifest import (  # noqa: E402,F401
...
def find_package_root_for_path(path: Path | None) -> Path | None:
    ...
    Returns the first ancestor directory that contains ``xinyu.yaml`` (or
    ``xinyu.yml``), or ``None`` if no such ancestor exists. Used to resolve
    ...
    for _ in range(20):  # safety bound against pathological paths
        if (current / "xinyu.yaml").exists() or (current / "xinyu.yml").exists():
            return current
...
def _load_manifest(pkg_dir: Path) -> dict:
    """Load xinyu.yaml manifest from a package directory."""
    manifest_file = pkg_dir / "xinyu.yaml"
    if not manifest_file.exists():
        manifest_file = pkg_dir / "xinyu.yml"
    if not manifest_file.exists():
