...
from xinyu_runtime.session.embedding import create_embedder
from xinyu_runtime.session.memory import SessionMemory
from xinyu_runtime.session.resume import (
    detect_session_type,
    resume_agent,
    resume_terrarium,
)
...
def _iter_session_files() -> list[Path]:
    files: list[Path] = []
    for session_dir in _session_dirs_for_read():
        if session_dir.exists():
            files.extend(session_dir.glob("*.xinyu"))
            files.extend(session_dir.glob("*.kt"))
    return files
