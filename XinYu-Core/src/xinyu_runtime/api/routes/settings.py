...
from xinyu_runtime.llm.codex_auth import CodexTokens, oauth_login, refresh_tokens
from xinyu_runtime.llm.codex_rate_limits import get_cached as get_cached_codex_usage
...
from xinyu_runtime.utils.paths import readable_path, xinyu_path
...
def _ui_prefs_path() -> Path:
    return xinyu_path("ui_prefs.json")


def _readable_ui_prefs_path() -> Path:
    return readable_path("ui_prefs.json")


_UI_PREF_DEFAULTS: dict[str, Any] = {
    "theme": "system",
    "xinyu-desktop-zoom": 1.15,
    "xinyu-mobile-zoom": 1.25,
    "nav-expanded": True,
    "xinyu-force-desktop": False,
    "xinyu.presets.user": {},
    "xinyu.layout.activePreset": None,
    "xinyu.layout.trees": {},
    "xinyu.layout.instances": {},
    "xinyu.splitPane": {},
}


def _load_ui_prefs() -> dict[str, Any]:
    path = _readable_ui_prefs_path()
    if not path.exists():
...
async def get_native_tools():
    ...
    """
    from xinyu_runtime.builtins.tool_catalog import list_provider_native_tools
...
def _mcp_config_path():
    return xinyu_path("mcp_servers.yaml")


def _readable_mcp_config_path():
    return readable_path("mcp_servers.yaml")


def _load_mcp_config():
    path = _readable_mcp_config_path()
    if not path.exists():
        return []
    try:
        with open(path, encoding="utf-8") as f:
