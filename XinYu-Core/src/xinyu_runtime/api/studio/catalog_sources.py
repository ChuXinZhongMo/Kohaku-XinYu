...
workspace-authored files, workspace ``xinyu.yaml`` manifest entries,
...
def load_workspace_manifest(ws: Any) -> dict:
    """Return the parsed ``xinyu.yaml`` for a workspace (or ``{}``)."""
    if ws is None:
        return {}
    root = getattr(ws, "root_path", None)
    if root is None:
        return {}
    for name in ("xinyu.yaml", "xinyu.yml"):
...
def manifest_entry(raw: dict, *, source: str, entry_type: str) -> dict:
    """Normalize a xinyu.yaml manifest entry into the catalog shape."""
    return {
        "name": raw.get("name", ""),
...
def classify_io(item: dict) -> str:
    """Classify a xinyu.yaml ``io:`` entry as input / output / unknown."""
...
def workspace_manifest_entries(ws: Any, kind: str) -> list[dict]:
    """Manifest entries from the open workspace's xinyu.yaml."""
