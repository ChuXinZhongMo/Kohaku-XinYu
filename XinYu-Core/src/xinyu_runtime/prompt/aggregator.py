...
from :mod:`xinyu_runtime.prompt.framework_hints`. Harness authors override
it per-creature via ``AgentConfig.framework_hint_overrides`` or per-package
via ``framework_hints:`` in ``xinyu.yaml``. Canonical override keys:
...
from xinyu_runtime.builtin_skills import get_all_subagent_docs, get_all_tool_docs
from xinyu_runtime.core.registry import Registry
from xinyu_runtime.parsing.format import (
    BRACKET_FORMAT,
    XML_FORMAT,
    ToolCallFormat,
    format_tool_call_example,
)
from xinyu_runtime.prompt.framework_hints import (
...
from xinyu_runtime.prompt.plugins import (
    BasePlugin,
    PluginContext,
    get_default_plugins,
)
from xinyu_runtime.prompt.template import render_template_safe
from xinyu_runtime.prompt.tool_contributions import build_tool_guidance_section
    ...
    *,
    ...
    Args:
        ...
        framework_hint_overrides: Map of canonical hint key -> replacement
            prose. See :mod:`xinyu_runtime.prompt.framework_hints` for the
...
def _build_output_hints(
    ...
    *,
    ...
    if known_outputs == {"default"}:
        logger.debug("Only default output registered, omitting wrapper-only output hints")
        return ""
