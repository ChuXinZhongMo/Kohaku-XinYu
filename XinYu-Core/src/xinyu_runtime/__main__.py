"""XinYu Runtime entry point.

When executed as ``python -m xinyu_runtime``, this module decides which path
to take:
...
def main() -> int:
    ...
    if _is_briefcase_bundle() and len(sys.argv) <= 1:
        from xinyu_runtime.__briefcase__ import main as briefcase_main

        briefcase_main()
        return 0

    from xinyu_runtime.cli import main as cli_main
