...
- live message time is now an explicit first-class prompt context: QQ payloads carry event Unix/ISO time, live and renderer prompts receive current runtime time, message event time, message age, and event-time source, renderer conversation tails preserve per-message `recorded_at`, and QQ event-time parsing now lives in `xinyu_qq_event_time.py` instead of the gateway main file.
