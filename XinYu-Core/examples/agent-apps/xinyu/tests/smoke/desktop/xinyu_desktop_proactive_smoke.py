...
async def _smoke() -> list[str]:
    ...
    with tempfile.TemporaryDirectory(prefix="xinyu-desktop-proactive-") as tmp:
        ...
        if "我让 Codex 帮忙那次" not in scrubbed_preview:
        ...
        if "我后面想反复想的是" not in scrubbed_reason or "我让 Codex 帮忙那次" not in scrubbed_reason:
        ...
        history = inbox.get("history", [])
        if not history or history[0].get("candidateId") != "proreq-smoke" or history[0].get("status") != "read_locally":
            failures.append(f"read-local proactive ack should remain reviewable in history: {history}")
        history_text = (xinyu_dir / "memory/context/proactive_request_history.jsonl").read_text(encoding="utf-8")
        if "proreq-smoke" not in history_text or "read_locally" not in history_text:
            failures.append("read-local proactive ack was not persisted to proactive history jsonl")
        ...
        history = inbox.get("history", [])
        if not history or history[0].get("status") != "answered" or history[0].get("desktopAction") != "reply":
            failures.append(f"reply proactive ack should update reviewable history: {history}")
