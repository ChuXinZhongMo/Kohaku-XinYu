...
def main() -> int:
    ...
    assert NativeQQGateway._should_split_visible_reply is xinyu_qq_reply_bubbles.gateway_should_split_visible_reply
    assert NativeQQGateway._should_split_outbox_visible_reply is xinyu_qq_reply_bubbles.gateway_should_split_outbox_visible_reply
    assert NativeQQGateway._split_visible_reply_bubbles is xinyu_qq_reply_bubbles.gateway_split_visible_reply_bubbles
    assert NativeQQGateway._visible_reply_bubbles is xinyu_qq_reply_bubbles.gateway_visible_reply_bubbles
    assert NativeQQGateway._outbox_visible_reply_bubbles is xinyu_qq_reply_bubbles.gateway_outbox_visible_reply_bubbles

    class TimeoutFallbackClient:
        async def chat(self, payload):
            raise BridgeError(
                'core bridge HTTP 504: {"accepted": false, "reply": "", '
                '"notes": ["bridge_request_timeout"]}'
            )
    ...
    class TimeoutFallbackGateway(NativeQQGateway):
        def __init__(self):
            ...
            self.client = TimeoutFallbackClient()
            self.replies = []
            self.traces = []
    ...
    async def _bridge_timeout_fallback_smoke():
        timeout_gateway = TimeoutFallbackGateway()
        await timeout_gateway._dispatch_prepared_message(
            None,
            PreparedMessage(
                target=ReplyTarget(message_kind="private", user_id="42", group_id=""),
                payload={
                    "text": "still there?",
                    "message_id": "bridge-timeout-smoke",
                    "session_id": "qq:private:42",
                    "metadata": {
        ...
        assert len(timeout_gateway.replies) == 1
        assert "\u6536\u5230" in timeout_gateway.replies[0]
        assert "\u8d85\u65f6" in timeout_gateway.replies[0]
        assert "core bridge HTTP" not in timeout_gateway.replies[0]
        assert any(
            item["stage"] == "dispatch_error" and item["drop_reason"] == "bridge_request_timeout"
            for item in timeout_gateway.traces
        )
    ...
    gated_coalesced, coalesced_decision = gateway._apply_owner_private_segmented_intent_gate(coalesced)
    assert coalesced_decision["action"] == "reply_now"
    assert gated_coalesced.payload["metadata"]["qq_segmented_intent_gate"] is True
    assert gated_coalesced.payload["metadata"]["qq_segmented_intent_action"] == "reply_now"
            ...
            self.traces.append(
                {
                    ...
                    "text": prepared.payload.get("text") if prepared is not None and isinstance(prepared.payload, dict) else "",
                    "metadata": metadata,
                }
            )
    ...
    def _owner_text_event(message_id: int, text: str) -> dict:
        event = dict(private_event)
    ...
    async def _owner_private_intent_gate_smoke():
        intent_gateway = IntentGateGateway()
        await intent_gateway._handle_onebot_event(
            None,
            _owner_text_event(1201, "\u6211\u60f3\u60f3\u529e\u6cd5\uff0c\u518d\u60f3\u60f3"),
            arrival_seq=1,
            session_queue_key="private:42",
        )
        assert intent_gateway.dispatched == []
        assert any(item["drop_reason"] == "owner_private_intent_silent" for item in intent_gateway.traces)

        await intent_gateway._handle_onebot_event(
            None,
            _owner_text_event(1202, "\u4e0d\u662f\uff0c\u6211\u7684\u610f\u601d\u662f\u8981\u628a\u5206\u6bb5\u5f53\u6210\u4e00\u4e2a\u610f\u56fe"),
            arrival_seq=2,
            session_queue_key="private:42",
        )
        assert len(intent_gateway.dispatched) == 1
        correction_metadata = intent_gateway.dispatched[-1].payload["metadata"]
        assert correction_metadata["qq_segmented_intent_action"] == "correction"
        assert correction_metadata["qq_should_reply"] is True

        await intent_gateway._handle_onebot_event(
            None,
            _owner_text_event(1203, "\u597d\uff0c\u90a3\u5c31\u8fd9\u6837\u6539"),
            arrival_seq=3,
            session_queue_key="private:42",
        )
        assert len(intent_gateway.dispatched) == 2
        task_metadata = intent_gateway.dispatched[-1].payload["metadata"]
        assert task_metadata["qq_segmented_intent_action"] == "task_instruction"
        assert task_metadata["qq_should_reply"] is True

    asyncio.run(_owner_private_intent_gate_smoke())
