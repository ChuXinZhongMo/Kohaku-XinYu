...
def main() -> int:
    failures: list[str] = []
    scratch = ROOT / "runtime/action_experience_digest_smoke_tmp"
    ...
    with tempfile.TemporaryDirectory(prefix="xinyu-action-digest-", dir=str(scratch)) as tmp:
        ...
        if exp_id not in dream_seeds or "minecraft_server" not in dream_seeds or "日志" not in dream_seeds:
        ...
        if exp_id not in reflection_queue or "我想把那次动作留下的感觉再想一遍" not in reflection_queue:
        ...
        followup = compose_action_digest_followup(root, "刚才有进入梦和反思吗？", max_age_seconds=30 * 24 * 3600)
        followup_reply = str((followup or {}).get("reply", ""))
        if not followup_reply or "进了" not in followup_reply or "编号" in followup_reply:
            failures.append("digest followup should answer dream/reflection routing")
        residue_followup = compose_action_digest_followup(root, "刚才那次留下了什么残留？", max_age_seconds=30 * 24 * 3600)
        residue_reply = str((residue_followup or {}).get("reply", ""))
        if "Java memory overflow" not in residue_reply and "memory overflow" not in residue_reply and "minecraft_server" not in residue_reply:
