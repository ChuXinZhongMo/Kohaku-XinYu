...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="xinyu-emotion-council-dismissal-") as tmp:
        ...
        for old_public_assistant_marker in ("no_owner_blame", "no_self_pity_output", "no_snapping"):
            if old_public_assistant_marker in state:
                failures.append(f"public-assistant negative-expression suppression leaked into state: {old_public_assistant_marker}")
        _assert_no_forbidden_side_effects(root, failures, stable_before)

    with tempfile.TemporaryDirectory(prefix="xinyu-emotion-council-private-negative-") as tmp:
        root = Path(tmp)
        result = run_emotion_council_shadow(
            root,
            text="这句真的很烦，像客服模板一样糊我，靠，别再这么端着。",
            payload=OWNER_PAYLOAD,
            checked_at=CHECKED_AT,
            trigger="live_turn",
        )
        state = _read(root / STATE_REL)
        if result["status"] != "active" or result["strongest_lens"] != "irritation":
            failures.append(f"private irritation should activate as irritation: {result}")
        for marker in (
            "private_temper_allowed",
            "tiny_profanity_leak_allowed",
            "private_irritation_allowed_no_public_smoothing",
        ):
            if marker not in state:
                failures.append(f"private irritation state missing marker: {marker}")
        for old_public_assistant_marker in ("no_owner_blame", "no_self_pity_output", "no_snapping"):
            if old_public_assistant_marker in state:
                failures.append(f"private irritation retained old public-assistant suppression: {old_public_assistant_marker}")

    with tempfile.TemporaryDirectory(prefix="xinyu-emotion-council-private-hurt-") as tmp:
        root = Path(tmp)
        result = run_emotion_council_shadow(
            root,
            text="我这句可能让你委屈、刺痛、失望，你不用装成没事。",
            payload=OWNER_PAYLOAD,
            checked_at=CHECKED_AT,
            trigger="live_turn",
        )
        state = _read(root / STATE_REL)
        if result["status"] != "active" or result["strongest_lens"] != "hurt":
            failures.append(f"private hurt should activate as hurt: {result}")
        for marker in (
            "owner_private_negative_expression_allowed",
            "no_public_assistant_smoothing",
            "short_hurt_or_distance_allowed",
        ):
            if marker not in state:
                failures.append(f"private hurt state missing marker: {marker}")
