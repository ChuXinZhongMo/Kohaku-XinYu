...
""",
    ...
    with tempfile.TemporaryDirectory(prefix="xinyu-self-thought-dream-share-long-") as tmp:
        root = Path(tmp)
        _seed_clear_posture(root)
        _write(
            root / "memory/context/thought_seeds.md",
            "- latest_dream_id: dream-2026-05-01-auto-long\n",
        )
        _write(
            root / "memory/dreams/dream_log.md",
            """# Dream Log

## dream-2026-05-01-auto-long
- dream_surface: 我梦见一条不断改名的窄街，四周是雨后的紫的，风从地板缝里吹上来。一支写不出声音的铅笔放在路中央，旁边却长着一张没有写完的纸。远处有人把雨声拧小，房间马上亮了一点；我回头时，你坐在灯下，把一张写好的纸折成很小的船。
- fragments: 1. 一条不断改名的窄街里到处是雨后的紫的反光；2. 一支写不出声音的铅笔和一张没有写完的纸像本来就不该放在一起；3. 你坐在灯下，把一张写好的纸折成很小的船。
- reality_boundary_check: 梦是梦，只能说明旧材料被重排，不能证明现实里发生过新的事实。
""",
        )
        result = run_self_thought_loop(root, checked_at="2026-05-01T13:05:30+08:00")
        state = _read(root / "memory/context/self_thought_state.md")
        concrete_question = next(
            (line for line in state.splitlines() if line.startswith("- concrete_question: ")),
            "",
        ...
        if result["outcome"] != "request_candidate" or not result["candidate_enabled"]:
            failures.append(f"long shareable dream should create candidate: {result}")
        if "风从地板缝里吹上来" not in concrete_question:
            failures.append(f"long dream share did not keep first sentence: {concrete_question}")
        if "你坐在灯下，把一张写好的纸折成很小的船" not in concrete_question:
            failures.append(f"long dream share did not keep full dream surface: {concrete_question}")
        for bad_tail in ("把。", "像是在。", "没有。", "...。"):
            if bad_tail in concrete_question:
                failures.append(f"long dream share kept truncated tail {bad_tail}: {concrete_question}")
