...
"""
    ...
    def _codex_status_reply(
        ...
        *,
        ...
        task_subject = self._codex_task_subject(task_text)
        variant_seed = "|".join(part for part in (task_subject, report_label, request_label, status) if part)
        if status == "started":
            return self._codex_reply_variant(
                variant_seed,
                (
                    self._codex_started_reply(task_subject, 0),
                    self._codex_started_reply(task_subject, 1),
                    self._codex_started_reply(task_subject, 2),
                ),
            )
        if status == "done":
            return self._codex_reply_variant(
    ...
    @classmethod
    def _codex_task_subject(cls, task_text: str) -> str:
        task = cls._codex_owner_task_text(task_text)
    ...
    def _codex_completion_outbox_message(
        ...
        *,
        ...
        if timed_out:
            head = self._codex_reply_variant(
                variant_seed,
                (
                    "Codex 卡住了，这次不算完成。",
                    "它那边超时了，先不算跑完。",
    ...
    def _enqueue_codex_completion_if_needed(
        ...
        *,
        ...
        for image_file in self._codex_generated_image_artifacts(result, task_text=text):
            enqueue_qq_outbox_image(
                self.xinyu_dir,
                user_id=user_id,
    ...
    def _codex_generated_image_artifacts(self, result: Any | None, *, task_text: str, limit: int = 3) -> list[Path]:
        if result is None or not self._looks_like_codex_image_generation_task(task_text):
            return []
        roots: list[Path] = []
        for path_text in (
