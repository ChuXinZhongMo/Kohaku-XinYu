...
def _prepare_quality_followup_case(root: Path) -> None:
    _prepare_case(root, quality_grade="review_needed", warning_count=2)
    _write(
        root / "memory/knowledge/source_requests.md",
        """---
title: Source Requests Activation Smoke Quality Followup
memory_type: source_requests
time_scope: short_term
subject_ids: [xinyu]
protected: false
...
- avoid_host: arxiv.org
- followup_slot: 1
- source_policy: controlled_fetch_only
- planned_at: 2026-04-24T00:00:00+08:00
- reason: source diversity follow-up for repeated host arxiv.org
""",
    )
...
def main() -> int:
    ...
    followup_result = {"activation_permission": "unknown", "allowed_queries": 0, "activation_reason": "unknown"}
    provider_followup_result = {"provider_results": 0, "pending_requests": 0}
...
""",
        ...
        try:
            try:
                ...
                _prepare_quality_followup_case(root)
                os.environ["XINYU_AUTONOMOUS_SEARCH"] = "enabled"
                followup_result = run_autonomous_search_activation(root, mode="activation_smoke_quality_followup")
                provider_followup_result = run_source_search_provider(
                    root,
                    mode="activation_smoke_provider_quality_followup",
                    require_activation=True,
                )
                ...
                print("quality_followup_permission:", followup_result["activation_permission"])
                print("quality_followup_reason:", followup_result["activation_reason"])
                print("provider_followup_results:", provider_followup_result["provider_results"])
    ...
    if args.require_activation and (
        ...
        or followup_result["activation_permission"] != "provider_allowed"
        or followup_result["activation_reason"] != "learning_quality_followup_search_allowed"
        or int(followup_result["allowed_queries"]) != 1
        or int(provider_followup_result["provider_results"]) <= 0
