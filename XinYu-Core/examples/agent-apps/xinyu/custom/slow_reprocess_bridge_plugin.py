"""Low-frequency runtime bridge for Xinyu slow reprocess."""
...
class SlowReprocessBridgePlugin(BasePlugin):
    name = "xinyu_slow_reprocess_bridge"
    priority = 99
    ...
    async def post_llm_call(
        ...
        try:
            ...
            result = run_slow_reprocess(
                ...
                mode="runtime_slow_reprocess",
            )
            self._ctx.set_state("slow_reprocess_last_run", checked_at)
            _trace(
                root,
                "runtime_slow_reprocess "
                f"reflection={result['reflection_count']} "
                f"dream={result['dream_count']} "
                f"archive={result['archive_count']} "
                f"top_topic={result['top_topic']}",
            )
