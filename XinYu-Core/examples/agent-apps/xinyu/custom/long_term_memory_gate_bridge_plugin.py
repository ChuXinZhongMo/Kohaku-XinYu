...
def _trace(root: Path, line: str) -> None:
    trace_path = root / "memory/archive/long_term_memory_gate_trace.log"
...
class LongTermMemoryGateBridgePlugin(BasePlugin):
    ...
    def _should_run(self, root: Path) -> tuple[bool, str]:
        ...
        if "- long_term_memory_gate: yes" not in recommendations:
            return False, "recommendation_not_yes"

        last_run = self._ctx.get_state("long_term_memory_gate_last_run")
        if last_run:
            try:
