...
def _trace(root: Path, line: str) -> None:
    trace_path = root / "memory/knowledge/source_search_provider_trace.log"
...
class SourceSearchProviderBridgePlugin(BasePlugin):
    ...
    def _should_run(self, root: Path) -> tuple[bool, str]:
        ...
        if "- source_search_provider: yes" not in recommendations:
            return False, "recommendation_not_yes"
        activation = _read(root / "memory/knowledge/autonomous_search_activation_state.md")
        if "- activation_permission: provider_allowed" not in activation:
            return False, "activation_not_allowed"
        last_run = self._ctx.get_state("source_search_provider_last_run")
        if last_run:
            try:
