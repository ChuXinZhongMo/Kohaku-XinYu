...
def _trace(root: Path, line: str) -> None:
    trace_path = root / "memory/reflection/reflection_output_trace.log"
...
class ReflectionOutputBridgePlugin(BasePlugin):
    ...
    def _should_run(self, root: Path) -> tuple[bool, str]:
        ...
        if "- reflection_output: yes" not in recommendations:
            return False, "recommendation_not_yes"

        last_run = self._ctx.get_state("reflection_output_last_run")
        if last_run:
            try:
