"""Shared inner-framework manifest for Xinyu.

This keeps layer ordering and deterministic sync targets in one place so
plugins, future writers, and validation scripts can refer to the same model.
"""

from __future__ import annotations

from xinyu_storage_paths import knowledge_ref

INNER_FRAMEWORK_LAYERS: list[dict[str, object]] = [
    {
        "name": "anchor",
        "files": [
            "memory/self/core.md",
            "memory/self/system_prompt_memory.md",
            "memory/self/personality_profile.md",
            "memory/self/mind_loop_policy.md",
            "memory/self/voice_profile_zh.md",
            "memory/self/boundaries.md",
            "memory/context/real_world_anchor_policy.md",
            "memory/context/life_month_slots.md",
            "memory/context/real_life_input_adapter_policy.md",
            "memory/context/memory_event_sourcing_policy.md",
            "memory/context/capability_zones_state.md",
            "memory/context/codex_delegation_policy.md",
            "memory/archive/retention_model.md",
            "memory/context/exploration_policy.md",
            "memory/reflection/reflection_policy.md",
            "memory/dreams/dream_policy.md",
        ],
        "update_mode": "rare",
    },
    {
        "name": "time_continuity",
        "files": [
            "memory/context/time_anchor.md",
            "memory/context/continuity_index.md",
            "memory/context/automation_policy.md",
            "memory/context/automation_state.md",
            "memory/context/runtime_bridge_state.md",
            "memory/context/inner_cycle_state.md",
            "memory/context/recent_context.md",
            "memory/context/runtime_rhythm.md",
            "memory/context/maintenance_plan.md",
            "memory/context/maintenance_targets.md",
            "memory/context/maintenance_recommendations.md",
            "memory/context/maintenance_dispatch_state.md",
            "memory/context/initiative_policy.md",
            "memory/context/initiative_state.md",
            "memory/context/proactive_presence_state.md",
            "memory/context/inner_sync_state.md",
            "memory/context/real_life_input_events.md",
            "memory/context/real_life_input_adapter_state.md",
            "memory/context/current_life_month_context.md",
        ],
        "update_mode": "every_meaningful_turn",
    },
    {
        "name": "immediate_feeling",
        "files": [
            "memory/emotions/taxonomy.md",
            "memory/emotions/current_state.md",
            "memory/emotions/event_log.md",
            "memory/context/unfinished_experiences.md",
        ],
        "update_mode": "every_meaningful_turn",
    },
    {
        "name": "relationship",
        "files": [
            "memory/people/owner.md",
            "memory/relationships/index.md",
            "memory/relationships/owner_patterns.md",
            "memory/relationships/vector_model.md",
        ],
        "update_mode": "every_meaningful_turn",
    },
    {
        "name": "self_narrative",
        "files": [
            "memory/self/narrative.md",
            "memory/self/mind_loop_state.md",
            "memory/self/voice_calibration_log.md",
            "memory/self/personality_change_state.md",
            "memory/self/ai_self_iteration_state.md",
            "memory/self/ai_self_iteration_review_state.md",
        ],
        "update_mode": "when_meaning_changes",
    },
    {
        "name": "question_exploration",
        "files": [
            "memory/context/active_questions.md",
            "memory/context/question_pipeline_state.md",
            "memory/context/question_states.md",
            "memory/context/exploration_queue.md",
            "memory/context/social_inquiry_candidates.md",
            knowledge_ref("general.md"),
            knowledge_ref("ai_domain.md"),
            knowledge_ref("social_inquiry_policy.md"),
            knowledge_ref("social_inquiry_policy_state.md"),
            knowledge_ref("social_inquiry_answers.md"),
            knowledge_ref("source_notes.md"),
            knowledge_ref("source_materials.md"),
            knowledge_ref("source_requests.md"),
            knowledge_ref("source_gate_state.md"),
            knowledge_ref("source_reliability_state.md"),
            knowledge_ref("source_integration_gate_state.md"),
            knowledge_ref("source_request_planner_state.md"),
            knowledge_ref("source_search_results.md"),
            knowledge_ref("source_search_resolver_state.md"),
            knowledge_ref("autonomous_search_activation_state.md"),
            knowledge_ref("research_loop_dry_run_state.md"),
            knowledge_ref("source_search_provider_state.md"),
            knowledge_ref("search_result_gate_state.md"),
            knowledge_ref("outward_source_state.md"),
            knowledge_ref("source_comparison_state.md"),
            knowledge_ref("learner_integration_state.md"),
            knowledge_ref("learning_quality_state.md"),
            knowledge_ref("source_registry.md"),
            knowledge_ref("integration_policy.md"),
        ],
        "update_mode": "conditional",
    },
    {
        "name": "slow_reprocessing",
        "files": [
            "memory/reflection/reflection_queue.md",
            "memory/reflection/reprocessing_state.md",
            "memory/reflection/consolidation_state.md",
            "memory/reflection/reflection_output_state.md",
            "memory/reflection/reflection_log.md",
            "memory/reflection/growth_log.md",
            "memory/dreams/dream_seeds.md",
            "memory/dreams/dream_output_state.md",
            "memory/dreams/dream_weight_state.md",
            "memory/dreams/dream_log.md",
            "memory/archive/archive_queue.md",
            "memory/archive/long_term_memory_gate_state.md",
            "memory/archive/retention_gate_state.md",
            "memory/archive/archive_output_state.md",
            "memory/archive/archive_commit_state.md",
            "memory/archive/compressed.md",
            "memory/archive/dormant.md",
            "memory/archive/archive_policy.md",
        ],
        "update_mode": "queued",
    },
]

DETERMINISTIC_SYNC_ORDER: list[str] = [
    "time_anchor",
    "recent_context",
    "current_state",
    "owner_profile",
    "relationship_index",
    "continuity_index",
    "owner_patterns",
    "self_narrative",
    "reflection_queue",
    "dream_seeds",
    "archive_queue",
    "maintenance_targets",
    "inner_sync_state",
    "unfinished_experiences",
    "active_questions",
    "initiative_state",
    "question_states",
    "exploration_queue",
]

SYNC_LAYER_NAMES: list[str] = [
    "anchor/time",
    "immediate_feeling",
    "relationship",
    "continuity",
    "self_narrative",
    "question_pipeline",
    "initiative_loop",
    "memory_event_sourcing",
    "reflection_queue",
    "dream_seeds",
    "archive_queue",
    "maintenance_targets",
    "reflection_output",
    "dream_output",
    "source_gate",
    "source_reliability",
    "source_integration_gate",
    "source_request_planner",
    "source_search_resolver",
    "autonomous_search_activation",
    "source_search_provider",
    "search_result_gate",
    "outward_source",
    "source_comparison",
    "learner_integration",
    "learning_quality",
    "ai_self_iteration_gate",
    "consolidation",
    "long_term_memory_gate",
    "retention_gate",
    "archive_output",
    "archive_commit",
    "personality_growth_gate",
]
