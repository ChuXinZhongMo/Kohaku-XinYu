...
def _trace(root: Path, line: str) -> None:
    trace_path = root / "memory/knowledge/outward_source_trace.log"
...
class OutwardSourceBridgePlugin(BasePlugin):
    ...
    def _should_run(self, root: Path) -> tuple[bool, str]:
        ...
        if "- outward_source: yes" not in recommendations:
            return False, "recommendation_not_yes"

        last_run = self._ctx.get_state("outward_source_last_run")
        if last_run:
            try:
