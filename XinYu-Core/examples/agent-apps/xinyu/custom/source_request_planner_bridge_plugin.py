...
def _trace(root: Path, line: str) -> None:
    trace_path = root / "memory/knowledge/source_request_planner_trace.log"
...
class SourceRequestPlannerBridgePlugin(BasePlugin):
    ...
    def _should_run(self, root: Path) -> tuple[bool, str]:
        ...
        if "- source_request_planner: yes" not in recommendations:
            return False, "recommendation_not_yes"

        last_run = self._ctx.get_state("source_request_planner_last_run")
        if last_run:
            try:
