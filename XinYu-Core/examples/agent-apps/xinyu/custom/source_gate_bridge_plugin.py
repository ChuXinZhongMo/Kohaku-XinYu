"""Low-frequency runtime bridge for Xinyu source gate."""
...
def _trace(root: Path, line: str) -> None:
    trace_path = root / "memory/knowledge/source_gate_trace.log"
...
class SourceGateBridgePlugin(BasePlugin):
    name = "xinyu_source_gate_bridge"
    priority = 101
    ...
    def _should_run(self, root: Path) -> tuple[bool, str]:
        ...
        if "- deferred: source_gate" not in dispatch:
            return False, "dispatch_not_deferred"

        recommendations = _read(root / "memory/context/maintenance_recommendations.md")
        if "- source_gate: yes" not in recommendations:
            return False, "recommendation_not_yes"

        last_run = self._ctx.get_state("source_gate_last_run")
        if last_run:
            try:
    ...
    async def post_llm_call(
        ...
        try:
            ...
            result = run_source_gate(
                ...
                mode="runtime_source_gate",
            )
            self._ctx.set_state("source_gate_last_run", checked_at)
            _trace(
                root,
                "runtime_source_gate "
