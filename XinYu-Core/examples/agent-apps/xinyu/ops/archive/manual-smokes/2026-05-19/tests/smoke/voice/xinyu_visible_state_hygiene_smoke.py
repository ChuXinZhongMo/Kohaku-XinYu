...
""",
        ...
        if "minecraft_server 的日志扫过" not in reflection or "没有做成" not in reflection or "有点压着" not in reflection:
            failures.append("reflection state did not get readable action wording")
        if "我后面想反复想的是" not in proactive or "我让 Codex 帮忙那次" not in proactive:
