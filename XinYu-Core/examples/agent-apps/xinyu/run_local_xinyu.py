...
def _load_run_module(repo_root: Path):
    ...
    run_path = src_root / "xinyu_runtime" / "cli" / "run.py"
    if not run_path.exists():
        raise FileNotFoundError(f"XinYu CLI run entry not found: {run_path}")
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        description="Run XinYu from the local framework source without package install.",
    )
    parser.add_argument(
        "--mode",
        choices=["cli", "plain", "tui"],
        default="cli",
        help="Interactive mode passed to the local agent CLI.",
    )
    parser.add_argument(
        "--log-level",
