...
def main() -> int:
    ...
    subprocess.run(
        [str(python_exe), str(xinyu_dir / "manual_inner_sync.py"), "--user", args.user, "--assistant", args.assistant],
        check=True,
    )
    subprocess.run([str(python_exe), str(xinyu_dir / "manual_question_pipeline.py")], check=True)
    subprocess.run([str(python_exe), str(xinyu_dir / "manual_slow_reprocess.py")], check=True)
    subprocess.run([str(python_exe), str(xinyu_dir / "manual_reflection_output.py")], check=True)
    subprocess.run([str(python_exe), str(xinyu_dir / "manual_source_gate.py")], check=True)
