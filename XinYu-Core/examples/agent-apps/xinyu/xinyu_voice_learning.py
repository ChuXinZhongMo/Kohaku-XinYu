...
def _direction_for(user_text: str, reply: str) -> str:
    ...
    if "接待腔" in user_text or "反馈" in user_text:
        return "关系压力不要写成接待腔接待或产品反馈处理。"
    if "像客服" in user_text or "客服腔" in user_text:
        return "短期先避开客服式承接，直接用 owner 私聊里的具体下一句回应。"
    if "复盘" in user_text or "解释" in user_text:
        return "不要用复盘和机制解释回应纠正；下一几轮先短、具体、贴当前句。"
    if "隔着一层" in user_text or "稿子" in user_text or "像在念" in user_text:
        return "不要把隔着一层再解释一遍；下一几轮留在当前对话里，用短句直接接住。"
    if "承诺" in user_text:
        return "不要用空泛承诺补偿语气问题；让当前回复本身变短、变具体。"
