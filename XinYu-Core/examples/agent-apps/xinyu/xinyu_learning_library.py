...
def decode_text(data: bytes) -> str:
    for encoding in ("utf-8-sig", "utf-8", "gb18030", "latin-1"):
        try:
            return repair_legacy_mojibake(data.decode(encoding))
        except UnicodeDecodeError:
            continue
    return repair_legacy_mojibake(data.decode("utf-8", errors="replace"))
...
def ocr_command_timeout_seconds() -> int:
    try:
...
def extract_text_from_bytes(data: bytes, filename: str, content_type: str) -> str:
    ...
    if is_odt_type(filename, content_type):
        return repair_legacy_mojibake(extract_odt_text(data))
    if is_image_type(filename, content_type):
        ocr_text = repair_legacy_mojibake(extract_ocr_text_from_bytes(data, filename, force=True))
        if ocr_text.strip() and not pdf_text_looks_garbled(ocr_text):
            return ocr_text
        return ""
...
def command_github(args: argparse.Namespace) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="xinyu-learning-github-") as tmp:
        ...
        claim = claim_from_text(combined_text, fallback_claim)
