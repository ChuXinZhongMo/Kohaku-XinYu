...
def _vision_model() -> str:
    return (
        os.environ.get("XINYU_IMAGE_VISION_MODEL", "").strip()
        or os.environ.get("XINYU_LLM_MODEL", "").strip()
        or "gpt-4o-mini"
    )
...
def _vision_api_key() -> str:
    return (
        os.environ.get("XINYU_IMAGE_VISION_API_KEY", "").strip()
...
def _image_data_uri(path: Path, payload: dict[str, Any]) -> tuple[str, str, list[str]]:
    try:
        max_bytes = _as_int(os.environ.get("XINYU_IMAGE_VISION_MAX_BYTES"), DEFAULT_MAX_IMAGE_BYTES)
        ...
        if suffix in ANIMATED_CONTACT_SHEET_SUFFIXES:
            ...
            if len(data) > max_bytes:
                return "", f"image_too_large:{len(data)}", animated_notes + [animated_error]
    ...
    if len(data) > max_bytes:
        return "", f"image_too_large:{len(data)}", []
...
def describe_image_with_vision(path: Path, payload: dict[str, Any], *, owner_text: str, ocr_text: str = "") -> tuple[str, list[str]]:
    ...
    request = urllib.request.Request(
        _vision_base_url() + "/chat/completions",
    ...
    if not isinstance(choices, list) or not choices:
        return "", ["vision_empty_choices"]
    message = choices[0].get("message") if isinstance(choices[0], dict) else None
    content = message.get("content") if isinstance(message, dict) else ""
    if isinstance(content, list):
        content = "\n".join(_safe_str(part.get("text")) for part in content if isinstance(part, dict))
    summary = re.sub(r"\s+", " ", _safe_str(content)).strip()
    if not summary:
        return "", ["vision_empty_summary"]
    notes.append("vision_summary_created")
    return summary[:1200], notes
