...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="xinyu-proactive-request-dream-disclaimer-") as tmp:
        temp_root = Path(tmp)
        target = temp_root / "memory/context"
        target.mkdir(parents=True, exist_ok=True)
        (target / "initiative_state.md").write_text(
        ...
        disclaimer = "\u6211\u77e5\u9053\u8fd9\u53ea\u662f\u68a6\uff0c\u4e0d\u662f\u73b0\u5b9e\u65b0\u53d1\u751f\u7684\u4e8b"
        dream_message = "\u6211\u521a\u624d\u6709\u4e2a\u68a6\u60f3\u8ddf\u4f60\u8bf4\uff1a\u50cf\u662f\u4e00\u6bb5\u6ca1\u6709\u95e8\u724c\u7684\u591c\u8def\u91cc\u3002" + disclaimer
        _write_request_preview(
            target,
            dream_message,
            kind="dream_share",
        ...
        preview = run_proactive_presence(
            temp_root,
            evaluated_at="2026-05-01T11:25:00+08:00",
            mode="smoke_proactive_request_dream_disclaimer",
        )
        if disclaimer in preview["candidate_message"]:
            failures.append(f"dream share disclaimer should be stripped: {preview}")
        if "\u6ca1\u6709\u95e8\u724c\u7684\u591c\u8def" not in preview["candidate_message"]:
            failures.append(f"dream share content should remain after stripping disclaimer: {preview}")
