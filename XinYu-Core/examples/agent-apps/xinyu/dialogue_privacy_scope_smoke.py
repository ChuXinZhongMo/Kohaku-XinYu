...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="xinyu-privacy-scope-") as tmp:
        ...
        archive_dialogue_turn(
            root,
            desktop_owner_payload,
            user_text="cross channel owner detail lives on desktop",
            assistant_reply="desktop answer",
            message_type="ordinary_owner_chat",
        )
        ...
        if "cross channel owner detail lives on desktop" not in owner_result.prompt_block:
            failures.append("owner-private recall did not bridge desktop and QQ owner sessions")
