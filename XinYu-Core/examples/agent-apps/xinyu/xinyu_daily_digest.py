...
updated_at: {_one_line(payload.get('generated_at'))}
status: active
...
## Current
- generated_at: {_one_line(payload.get('generated_at'))}
...
def run_daily_digest_maintenance(
    ...
    *,
    ...
    if _field(source_text, "status") != "fetched":
        ...
        _append_trace(root, {"event_kind": "source_not_ready", "observed_at": observed})
        return result
    ...
    if not items:
        ...
        _append_trace(root, {"event_kind": "no_items", "observed_at": observed})
        return result

    digest = _source_digest(items)
    existing = _read_json(root / DIGEST_REL)
    if (
    ...
    _write_json(root / DIGEST_REL, payload)
...
def build_daily_digest_prompt_block(root: Path, *, limit: int = 900) -> str:
    data = _read_json(root.resolve() / DIGEST_REL)
