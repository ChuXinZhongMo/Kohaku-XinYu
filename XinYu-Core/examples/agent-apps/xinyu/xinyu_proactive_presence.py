...
DREAM_SHARE_DISCLAIMER_RE = re.compile(
    r"\s*(?:。|\.|！|!|？|\?)?\s*我知道这只是梦，不是现实新发生的事[。.!！]*\s*"
)
...
def _request_preview_candidate(proactive_request: str) -> dict[str, str]:
    ...
    if kind == "dream_share":
        candidate = _clean_dream_share_candidate(candidate)
    if candidate in {"", "none", "unknown"}:
        return {
            "candidate": "none",
...
def _clean_dream_share_candidate(candidate: str) -> str:
    cleaned = DREAM_SHARE_DISCLAIMER_RE.sub("", _one_line(candidate)).strip()
    return cleaned or _one_line(candidate)
